...
$name = $address = $salary = $department = $postion = "";
...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($input_department)){
        ...
        // Validate position
    $input_address = trim($_POST["position"]);
    ...
    if(empty($name_err) && empty($address_err) && empty($salary_err) && empty($department_err) && empty($position_err)){
        ...
        $sql = "UPDATE employees SET name=?, address=?, salary=? , department=?, position=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssi", $param_name, $param_address, $param_salary, $param_department, $param_position, $param_id);
